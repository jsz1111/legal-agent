...
同时提取用户明确说出的客观案情事实（金额、持续时间、身份关系、行为经过）、地区和时间信息。
只提取原文明确内容，不做推测；长期记忆与本轮冲突时以本轮为准。

填入 issues、domain、facts、region、time_info 字段。无明确问题时 issues 为空列表。"""
...
4. collected_facts 只提取用户明确说出的事实，不推测；用户更正前述信息时以“更正：...”记录。
